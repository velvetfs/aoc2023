my_str = """tsgbzmgbonethreedrqzbhxjkvcnm3
...
#     nums = [c for c in word if c.isdigit()]
#     print(nums)
#     numm = int(nums[0] + nums[-1])
#     print(numm)
#     num += numm
#
# print(num)
...
for word in my_str.split("\n"):
    ...
    for key in num_map:
        if key in word:
            word_mapleft[key] = word.find(key)
            word_mapright[key] = word.rfind(key)

    nums = [c for c in word if c.isdigit()]
    for n in nums:
        word_mapleft[n] = word.find(n)
        word_mapright[n] = word.rfind(n)

    word_mapleft = sorted(word_mapleft.items(), key=lambda x: x[1])
    word_mapright = sorted(word_mapright.items(), key=lambda x: x[1])

    numm = (num_map[word_mapleft[0][0]] if word_mapleft[0][0] in num_map else word_mapleft[0][0]) + (
        num_map[word_mapright[-1][0]] if word_mapright[-1][0] in num_map else word_mapright[-1][0])
