...
fn part1() {
    ...
    for line in my_string.lines() {
        ...
        let number = numbers[0].to_string() + &numbers[numbers.len() - 1].to_string();
...
fn part2() {
    ...
    let mut total = 0;
    for line in my_string.lines() {
        let mut number_idxs = Vec::new();
        let mut word_idxs_left = Vec::new();
        let mut word_idxs_right = Vec::new();
        // enumerate over the chars in the word with index
        for (i, c) in line.chars().enumerate() {
            if c.is_digit(10) {
                number_idxs.push((c.to_string(), i));
            }
        }

        for word in map.keys() {
            if line.contains(word) {
                word_idxs_left.push((map.get(word).unwrap().to_string(), line.find(word).unwrap()));
                word_idxs_right.push((map.get(word).unwrap().to_string(), line.rfind(word).unwrap()));
            }
        }

        let mut new_vec = Vec::new();
        new_vec.extend(number_idxs);
        new_vec.extend(word_idxs_left);
        new_vec.extend(word_idxs_right);
        new_vec.sort_by(|a, b| a.1.cmp(&b.1));

        let number = new_vec[0].0.to_string() + &new_vec[new_vec.len() - 1].0.to_string();
        let number: u32 = number.parse().unwrap();
        total += number;
    }
    println!("total: {}", total);
...
fn get_string() -> String {
    return r#"tsgbzmgbonethreedrqzbhxjkvcnm3
