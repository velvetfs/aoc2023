...
my_str = """Game 1: 10 green, 5 blue; 1 red, 9 green, 10 blue; 5 blue, 6 green, 2 red; 7 green, 9 blue, 1 red; 2 red, 10 blue, 10 green; 7 blue, 1 red
...
def part1():
    ...
    for line in my_str.splitlines():
        ...
        color_nums = re.findall(r"(\d+) (\w+)", line)
        for color_num, color in color_nums:
            if int(color_num) > limits[color]:
                break
        else:
            possible.append(int(game_num))
...
def part2():
    ...
    for line in my_str.splitlines():
        ...
        color_maxes = {}
        for color_num, color in color_nums:
            color_maxes[color] = max(color_maxes.get(color, 0), int(color_num))
