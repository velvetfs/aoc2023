...
def part1():
    ...
    for line in lines:
        ...
        for match in matches:
            num_match_dict[match.start()] = {"num": match.group()}
            if match.end()-1 not in num_match_dict:
                num_match_dict[match.end()-1] = {"start": match.start()}
        num_matches.append(num_match_dict)

        symbol_idx_list = []
        matches = re.finditer(r"[^\w.\n]", line)
        for match in matches:
            symbol_idx_list.append(match.start())
        symbol_idx_lists.append(symbol_idx_list)

    for idx, line in enumerate(lines):
        lines_to_check = lines[max(0, idx-1):min(len(lines), idx+2)]
        for symbol_idx in symbol_idx_lists[idx]:
            idxs_to_check = [max(0, symbol_idx-1), symbol_idx, min(len(line), symbol_idx+1)]
            for idx2, line_to_check in enumerate(lines_to_check):
                line_to_check_idx = idx + (idx2-1 if idx != 0 else idx2)
                for idx_to_check in idxs_to_check:
                    num_match_dict = num_matches[line_to_check_idx]
                    if idx_to_check in num_match_dict:
                        start_idx = num_match_dict[idx_to_check].get("start", idx_to_check)
                        found_num_tup = (line_to_check_idx, start_idx)
                        if start_idx in num_match_dict and found_num_tup not in found_nums:
                            found_nums.add(found_num_tup)
                            num = num_match_dict[start_idx]["num"]
                            nums.append(int(num))
