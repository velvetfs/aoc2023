...
// .1*1.598.."#;
    ...
    for (line_idx, line) in lines.enumerate() {
        ...
        let mut star_idxs = HashMap::new();
        for m in star_re.find_iter(line) {
            star_idxs.insert(m.start(), HashSet::new());
        }
        star_idxs_lists.push_back(star_idxs);
        let star_idx_len = star_idxs_lists.len();
        for (idx, star_idxs_map) in star_idxs_lists.iter_mut().enumerate() {
            for star_idx in star_idxs_map.keys().cloned().collect::<Vec<usize>>() {
                let idxs_to_check = vec![0.max(star_idx - 1), star_idx, (line.len() - 1).min(star_idx + 1)];
                for number_dict in &number_dicts.range((star_idx_len - (idx+1))..).cloned().collect::<VecDeque<HashMap<usize, HashMap<&str, usize>>>>() {
                    for idx in idxs_to_check.clone() {
                        if number_dict.contains_key(&idx) {
                            ...
                            star_idxs_map.get_mut(&star_idx).unwrap().insert((line_idx, word_start_idx, *number_dict.get(&word_start_idx).unwrap().get("val").unwrap()));
        ...
        if star_idxs_lists.len() > 1 {
            let star_idxs_map = star_idxs_lists.pop_front().unwrap();
            for neighbors in star_idxs_map.values() {
                println!("star_idxs_lists {:?}", neighbors);
                if neighbors.len() == 2 {
                    println!("gear_ratio {}", gear_ratio);
                    gear_ratio += neighbors.iter().map(|x| x.2).product::<usize>();
                }
            }
        }
        if number_dicts.len() > 1 {
            number_dicts.pop_front();
        }
    }

    for star_idxs_map in star_idxs_lists {
        for neighbors in star_idxs_map.values() {
            if neighbors.len() == 2 {
                gear_ratio += neighbors.iter().map(|x| x.2).product::<usize>();
            }
